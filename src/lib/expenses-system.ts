...
export function getExpenseMeta(exp: Expense): ExpenseMeta {
  ...
  return {
    ...
    costCenter: fromNotes.costCenter || fromStore?.costCenter,
    autoGenerated: fromNotes.autoGenerated || fromStore?.autoGenerated,
  };
}
...
export function advanceDueDate(item: RecurringExpense, fromDate = new Date()): string {
  ...
  if (item.frequency === "monthly") {
    ...
    next.setDate(Math.min(item.dayOfMonth || 1, 28));
...
/**
 * تنفيذ قالب دوري فعلياً: إنشاء مصروف بسند متسلسل، تحديث تاريخ الاستحقاق.
 * يُستخدم من الزر اليدوي ومن المولّد التلقائي.
 */
export async function executeRecurringExpense(
  item: RecurringExpense,
  deps: {
    addExpense: (exp: { amount: number; category: string; expenseDate: string; notes: string }) => Promise<unknown>;
  ...
  const branch = deps.branches?.find((b) => b.id === item.branchId);
  const today = new Date().toISOString().slice(0, 10);

  const voucher = issueVoucherNumber({
    amount: item.amount,
  ...
  const meta: ExpenseMeta = {
    accountId: item.accountId,
    accountName: acc?.name || "الدرج الرئيسي (كاش)",
    ...
    autoGenerated: !!deps.auto,
  };

  const monthLabel = new Date().toLocaleDateString("ar-EG", { month: "long", year: "numeric" });
  const notes = encodeExpenseNotes(
    `${item.title} — ${deps.auto ? "صرف تلقائي مجدول" : "صرف دوري مجدول"} (${monthLabel})`,
    meta
  );

  await deps.addExpense({ amount: item.amount, category: item.category, expenseDate: today, notes });

  updateRecurringExpense(item.id, {
    lastGeneratedDate: today,
    nextDueDate: advanceDueDate(item),
  });
...
/**
 * المولّد التلقائي: يمر على القوالب المفعّلة ذات الاعتماد التلقائي والمستحقة
 * ويسجلها كمصروفات فعلية. يعمل مرة واحدة يومياً عند فتح التطبيق.
 */
export async function runRecurringAutoGeneration(deps: {
  addExpense: (exp: { amount: number; category: string; expenseDate: string; notes: string }) => Promise<unknown>;
  branches?: Array<{ id: string; name: string }>;
  force?: boolean;
}): Promise<{ generated: RecurringExpense[]; pending: RecurringExpense[] }> {
  const today = new Date().toISOString().slice(0, 10);
  const lastRun = readStorage<string>(STORAGE_KEYS.RECURRING_AUTORUN, "");
  const list = getRecurringExpenses();

  const due = list.filter((r) => r.active && checkRecurringStatus(r).isDue && checkRecurringStatus(r).daysRemaining <= 0);
  const pending = due.filter((r) => !r.autoApprove);
  const generated: RecurringExpense[] = [];

  if (lastRun === today && !deps.force) {
    return { generated, pending };
  }

  for (const item of due) {
    if (!item.autoApprove) continue;
    if (item.lastGeneratedDate === today) continue;
    try {
      await executeRecurringExpense(item, { ...deps, auto: true });
      generated.push(item);
    } catch (e) {
      console.error("Auto recurring expense failed:", item.title, e);
    }
  }

  writeStorage(STORAGE_KEYS.RECURRING_AUTORUN, today);
  return { generated, pending };
}
