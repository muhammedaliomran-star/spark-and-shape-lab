...
export default function StorefrontPage() {
  ...
  return <AppShell><PageTransition><div className="space-y-6 pb-20" dir="rtl">
    ...
    {loading ? <div className="py-20 text-center text-muted-foreground"><Loader2 className="mx-auto mb-3 h-7 w-7 animate-spin" />جارٍ تجهيز متجرك…</div> : <Tabs defaultValue="setup">
      ...
      <TabsContent value="orders" className="mt-5"><div className="grid gap-3">{orders.map((order) => <BezelCard key={order.id} className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold">طلب #{order.public_number} · {order.customer_name}</p><p className="mt-1 text-sm text-muted-foreground">{order.customer_phone} · {fmt(order.total)} ج.م · {statusLabel[order.status]}</p></div>{["submitted", "under_review", "needs_info"].includes(order.status) && <div className="flex gap-2"><Button size="sm" onClick={() => updateOrder(order, "accepted")}><CheckCircle2 className="h-4 w-4" /> قبول وحجز</Button><Button size="sm" variant="outline" onClick={() => updateOrder(order, "rejected")}><XCircle className="h-4 w-4" /> رفض</Button></div>}{order.status === "accepted" && <div><Button size="sm" onClick={() => invoiceOrder(order)} disabled={order.order_type !== "cash_on_delivery"} title={order.order_type !== "cash_on_delivery" ? "طلب التقسيط يحتاج اتفاق الأقساط أولًا" : undefined}><ReceiptText className="h-4 w-4" /> إنشاء فاتورة وشحنة</Button>{order.order_type !== "cash_on_delivery" && <p className="mt-1 text-xs text-muted-foreground">طلب تقسيط — افتح فاتورة وحدد شروط الاتفاق أولًا.</p>}</div>}</div><p className="mt-3 text-sm text-muted-foreground">{order.delivery_area ? `${order.delivery_area} · ` : ""}{order.delivery_address}</p></BezelCard>)}{orders.length === 0 && <BezelCard className="p-10 text-center text-muted-foreground">لسه مفيش طلبات من المتجر.</BezelCard>}</div></TabsContent>
    </Tabs>}
